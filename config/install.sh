...
function setup_chatwoot() {
  ...
  git clone https://github.com/Eficaz-Sistemas/chatwoot.git
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  pnpm i
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/Eficaz-Sistemas/chatwoot/develop/deployment/nginx_chatwoot.conf
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/Eficaz-Sistemas/chatwoot/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://raw.githubusercontent.com/Eficaz-Sistemas/chatwoot/master/config/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
